...
/// When search is active, shows the search input bar instead.
/// When a filter is active (but not typing), shows the filter indicator.
fn render_legend(frame: &mut Frame, area: Rect, search_query: &str, search_active: bool) {
...
pub fn render_ui(
    ...
    // ----- Legend -----
    render_legend(frame, legend_area, search_query, search_active);
}
